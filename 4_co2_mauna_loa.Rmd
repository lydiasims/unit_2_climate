...
co2_20200to2025=co2[co2$decimal_date>2020, ]
co2_20200to2025=co2[which(co2$decimal_date>2020), ]
summary(co2_20200to2025)

plot(seasonal_cycle ~ decimal_date, data=co2_20200to2025, type='l')


jan_anomalies= co2$seasonal_cycle[which(co2$month==1)]
summary(jan_anomalies)
...
So to grab the last 5 years of data (plus a few months), we only want values of `co2$decimal_date` greater than 2018. We can use a conditional statement `>` to signal values *greater than* 2018:

```{r}
# Showing 2 ways to subset
summary(co2$decimal_date > 2018) # vector of TRUES and FALSES
summary(which(co2$decimal_date > 2018)) # vector of indices that meet condition (see ?which for bells and whistles)

# Either method can be used to subset the data
co2_2018to2020 = co2[co2$decimal_date > 2018,]
co2_2018to2020 = co2[which(co2$decimal_date > 2018),]

plot(seasonal_cycle ~ decimal_date, type="l", data=co2_2018to2020) 
```
